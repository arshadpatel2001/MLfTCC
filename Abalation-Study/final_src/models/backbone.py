...
class TaskHeads(nn.Module):
    """
    Four-task prediction heads sharing the same backbone representation z:
      1. Intensity change classification  (n_intensity classes, default 4)
      2. Direction classification          (n_direction classes, default 8)
      3. Intensity regression              (2 outputs: wind_norm, pres_norm)
         Predicts 24h-ahead wind speed and central pressure (normalized units).
      4. Track forecasting regression      (8 outputs: 4 steps × 2 coords)
         Predicts future [LONG_norm, LAT_norm] at t+6h, t+12h, t+18h, t+24h.
         Output reshaped to (B, 4, 2) — same prediction horizon as head 3.
    """

    def __init__(self, in_dim: int = 128, n_intensity: int = 4, n_direction: int = 8,
                 dropout: float = 0.1, pred_len: int = 4):
        super().__init__()
        ...
        # Track forecasting head: predicts future [LONG_norm, LAT_norm] at
        # t+6h, t+12h, t+18h, t+24h — i.e. pred_len × 2 values.
        # Uses the same backbone z, exactly like the other heads.
        # NaN steps are masked in track_loss(); full (B, pred_len, 2) is output.
        self.track_head = nn.Sequential(
            nn.Linear(in_dim, 128),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(128, pred_len * 2),  # pred_len × [long_norm, lat_norm]
        )
    ...
    def forward(self, z: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            ...
            pred_track:        (B, pred_len, 2)   future [long,lat] positions
        """
        B = z.shape[0]
        pred_track = self.track_head(z).view(B, self.pred_len, 2)
        return (
...
class TropiCycloneModel(nn.Module):
    ...
    def forward(self, batch: dict) -> dict:
        ...
        logits_int, logits_dir, pred_reg, pred_track = self.heads(feat["z"])
        return {
            **feat,
            "logits_intensity":  logits_int,
            "logits_direction":   logits_dir,
            "pred_intensity_reg": pred_reg,    # (B, 2): [wind_norm, pres_norm]
            "pred_track":         pred_track,  # (B, pred_len, 2): [long,lat] at t+6h..t+24h
        }
