...
def _haversine_km(
    lat1_deg: torch.Tensor, lon1_deg: torch.Tensor,
    lat2_deg: torch.Tensor, lon2_deg: torch.Tensor,
) -> torch.Tensor:
    """Great-circle distance in km. All inputs same shape (degrees)."""
    d2r = math.pi / 180.0
    dlat = (lat2_deg - lat1_deg) * d2r
    dlon = (lon2_deg - lon1_deg) * d2r
    a = (torch.sin(dlat / 2) ** 2
         + torch.cos(lat1_deg * d2r) * torch.cos(lat2_deg * d2r)
         * torch.sin(dlon / 2) ** 2)
    return _EARTH_R_KM * 2 * torch.asin(torch.sqrt(a.clamp(0.0, 1.0)))
...
class BasinResult:
    ...
    # ── Track forecasting metrics (ADE / FDE in km) ───────────────────────────
    # Average Displacement Error: mean haversine distance over all valid steps.
    # Final Displacement Error:   haversine distance at the last predicted step.
    # ade_per_step: per-step ADE list [t+6h, t+12h, t+18h, t+24h].
    # NaN-filled steps in y_track_norm are excluded from all computations.
    ade_km:       float      = 0.0    # mean ADE over all valid future steps
    fde_km:       float      = 0.0    # ADE at the final step (t+24h)
    ade_per_step: List[float] = field(default_factory=list)  # one entry per pred step
    n_track_samples: int = 0          # number of valid track prediction steps
...
class BasinEvaluator:
    ...
    def __init__(self, basin_name: str = "unknown", collect_features: bool = False):
        ...
        # Track forecasting: pred_track (B,4,2) and y_track_norm (B,4,2)
        # Coordinates are [LONG_norm, LAT_norm] ∈ [0,1]; haversine applied in compute().
        self._preds_track  : List[torch.Tensor] = []  # (B, pred_len, 2) normalized
        self._labels_track : List[torch.Tensor] = []  # (B, pred_len, 2) normalized, may have NaN
    ...
    def update(self, batch: dict, out: dict):
        ...
        # ── Track forecasting ─────────────────────────────────────────────────
        if "pred_track" in out:
            self._preds_track.append(out["pred_track"].detach().cpu())   # (B, P, 2)
        if "y_track_norm" in batch:
            self._labels_track.append(batch["y_track_norm"].detach().cpu())  # (B, P, 2)
    ...
    def compute(self) -> BasinResult:
        if not self._preds_int:
            return BasinResult(
                ...
                n_reg_samples=0,
                ade_km=0.0, fde_km=0.0, ade_per_step=[], n_track_samples=0,
            )
        ...
        # ── Track forecasting ADE / FDE ───────────────────────────────────────
        ade_km = fde_km = 0.0
        ade_per_step: List[float] = []
        n_track = 0

        if self._preds_track and self._labels_track:
            pt = torch.cat(self._preds_track,  dim=0)   # (N, P, 2) normalized [long,lat]
            lt = torch.cat(self._labels_track, dim=0)   # (N, P, 2) normalized, may have NaN

            # Convert normalized → degrees
            pred_lon = pt[..., 0] * 360.0 - 180.0      # (N, P)
            pred_lat = pt[..., 1] * 180.0 - 90.0
            true_lon = lt[..., 0] * 360.0 - 180.0
            true_lat = lt[..., 1] * 180.0 - 90.0

            # Valid mask: step is valid only when both lat and lon are finite
            valid = torch.isfinite(lt).all(dim=-1)      # (N, P)

            step_errors = _haversine_km(true_lat, true_lon, pred_lat, pred_lon)  # (N, P)

            # Per-step ADE (average only over valid samples)
            P = pt.shape[1]
            for s in range(P):
                m = valid[:, s]
                if m.any():
                    ade_per_step.append(float(step_errors[:, s][m].mean().item()))
                else:
                    ade_per_step.append(float("nan"))

            # Global ADE: mean over all valid (sample, step) pairs
            if valid.any():
                ade_km = float(step_errors[valid].mean().item())
                fde_km = ade_per_step[-1] if ade_per_step else 0.0
                n_track = int(valid.sum().item())
        ...
        return BasinResult(
            ...
            ade_km           = ade_km,
            fde_km           = fde_km,
            ade_per_step     = ade_per_step,
            n_track_samples  = n_track,
        )
    ...
    def reset(self):
        ...
        self._preds_track.clear()
        self._labels_track.clear()
