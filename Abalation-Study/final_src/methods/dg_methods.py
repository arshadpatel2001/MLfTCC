...
def task_loss(
    ...
    # Track forecasting (optional — ignored when None or track_weight == 0)
    pred_track:    Optional[torch.Tensor] = None,  # (B, 4, 2): predicted [long,lat] at t+6h..t+24h
    y_track_norm:  Optional[torch.Tensor] = None,  # (B, 4, 2): target   [long,lat], NaN for missing steps
    track_weight:  float = 0.5,
) -> torch.Tensor:
    """
    Combined task loss:
      L = int_weight  * CE(intensity_cls)
        + dir_weight  * CE(direction_cls)
        + reg_weight  * MSE(wind_reg, pres_reg)     [only when targets available]
        + track_weight* MSE(pred_track, y_track)    [only when targets available]

    NaN masking applies to both regression and track terms — end-of-storm steps
    that have no future row are excluded from gradient computation.

    The IRM/PhysIRM gradient penalty is applied to classification logits only
    (not regression or track), consistent with the original IRM formulation.
    """
    l_int = F.cross_entropy(logits_int, y_int)
    l_dir = F.cross_entropy(logits_dir, y_dir)
    loss  = int_weight * l_int + dir_weight * l_dir

    # ── Intensity regression term ─────────────────────────────────────────────
    ...
    # ── Track forecasting term ────────────────────────────────────────────────
    # pred_track: (B, 4, 2)  — model output from track_head
    # y_track_norm: (B, 4, 2) — [long_norm, lat_norm] at t+6h..t+24h, NaN for missing
    if (pred_track is not None
            and y_track_norm is not None
            and track_weight > 0.0):
        step_mask = torch.isfinite(y_track_norm).all(dim=-1)   # (B, 4) — valid steps
        if step_mask.any():
            err = F.mse_loss(pred_track, y_track_norm, reduction="none")  # (B, 4, 2)
            err = err.mean(dim=-1)                                          # (B, 4)
            l_track = err[step_mask].mean()
            loss    = loss + track_weight * l_track
...
def per_env_loss(
    ...
    **kwargs,
    ...
    """Compute full task loss on a single environment (basin) batch.

    Includes intensity/direction classification, intensity regression, and
    track forecasting — all from one model(batch) call.
    """
    out = model(batch)
    return task_loss(
...
class ERM(DGMethod):
    ...
    def __init__(self, reg_weight: float = 0.5, track_weight: float = 0.5):
...
class IRM(DGMethod):
    ...
    def __init__(self, irm_lambda: float = 1.0, warmup_steps: int = 500,
                 reg_weight: float = 0.5, track_weight: float = 0.5):
        """
        Args:
            ...
            track_weight:  Weight on the track forecasting MSE loss.
        """
        ...
        for _ in range(self.inner_steps):
            ...
            with torch.autocast(device_type=device_type, enabled=use_amp):
                ...
                loss = task_loss(
                    ...
                    reg_weight   = self.reg_weight,
                    pred_track   = out.get("pred_track"),
                    y_track_norm = support_batch.get("y_track_norm"),
                    track_weight = self.track_weight,
                )
        ...
        for basin, batch in batches.items():
            ...
            with torch.autocast(device_type=device_type, enabled=use_amp):
                ...
                loss_q = task_loss(
                    ...
                    reg_weight   = self.reg_weight,
                    pred_track   = out_q.get("pred_track"),
                    y_track_norm = query.get("y_track_norm"),
                    track_weight = self.track_weight,
                )
...
class PhysIRM(DGMethod, nn.Module):
    ...
    def __init__(
        ...
        track_weight:  float = 0.5,    # weight on track forecasting MSE loss
    ):
        nn.Module.__init__(self)
        self.irm_lambda   = irm_lambda
    ...
    def compute_loss(self, batches: Dict[str, dict], model: nn.Module):
        ...
        for basin, batch in batches.items():
            ...
            logits_phys_int, logits_phys_dir, _, _ = model.heads(z_phys_only)
